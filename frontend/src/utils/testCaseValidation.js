...
export const validateAndFixTestCasesForAPI = (testCases) => {
  ...
  testCases.forEach((testCase, index) => {
    ...
    delete fixedTestCase.key; // AI-generated key field
    delete fixedTestCase.id; // Will be auto-generated by backend
    delete fixedTestCase.problemId; // Will be set by backend

    // Ensure epsilon and dependsOn are null if not provided (backend expects null, not undefined)
    if (fixedTestCase.epsilon === undefined) {
      fixedTestCase.epsilon = null;
    }
    if (fixedTestCase.dependsOn === undefined) {
      fixedTestCase.dependsOn = null;
    }
